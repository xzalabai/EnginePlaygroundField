...
void Graphics::DrawLine(Vec2<float> p0, Vec2<float> p1, Color color)
{
	float slope = 0.0f;
	if (p1.X() != p0.X())
	{
	...
	if (p1.X() != p0.X() && abs(slope) <= 1.0f)
	{
		...
		const float b = p0.Y() - (slope * p0.X());

		for (int x = (int)p0.X(); x <= (int)p1.X(); x++)
		{
			const int y = (int)(slope * x + b);
	...
	else
	{
		...
		const float slopeInvers = (p1.X() - p0.X()) / (p1.Y() - p0.Y());
		const float p = p0.X() - (slopeInvers * p0.Y());

		for (int y = (int)p0.Y(); y <= (int)p1.Y(); y++)
		{
			const int x = (int)(slopeInvers * y + p);
			if (x >= 0 && x < ScreenWidth && y >= 0 && y < ScreenHeight)
			{
				PutPixel(x, y, color);
			}
